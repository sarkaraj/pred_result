...
from support_func import _get_dt_frm_b_date, string_to_gregorian
...
def _get_invoice_data(sqlContext, **kwargs):
    ...
    _invoice_raw_data = sqlContext.sql(_query) \
        ...
        .select(col('customernumber'),
                ...
                (max(udf(string_to_gregorian, DateType())(col('last_delivery_date')))).over(
                    window=Window.partitionBy("customernumber").orderBy('last_delivery_date').rangeBetween(-sys.maxsize,
                                                                                                           sys.maxsize)).cast(
                    StringType()).alias('salesrep_store_visit')
                ) \
        .drop(col('b_date')) \
        .drop(col('bill_date'))
